...
function quebrarTextoInteligente(texto: string, maxCaracteres: number): string[] {
  ...
  while (textoRestante.length > 0) {
    ...
    for (const separador of separadores) {
      ...
      if (ultimaOcorrencia > maxCaracteres * 0.7) { // Só aceita se for pelo menos 70% da linha
...
export function getDadosAdicionais({
  ...
  // Desenhar bordas da seção
  linhaHorizontal({ x1: 0, x2: 0, y: finalEspacoDet + 8, doc, ajusteX, ajusteY, margemDireita, margemEsquerda, margemTopo });
  linhaHorizontal({ x1: 0, x2: 0, y: alturaSecao, doc, ajusteX, ajusteY, margemDireita, margemEsquerda, margemTopo });
  linhaVertical({ y1: finalEspacoDet + 8, y2: alturaSecao, x: 0, doc, ajusteX, ajusteY, margemEsquerda, margemTopo });
  linhaVertical({ y1: finalEspacoDet + 8, y2: alturaSecao, x: 367, doc, ajusteX, ajusteY, margemEsquerda, margemTopo }); // MOC Z02: 12,95 cm
  linhaVertical({ y1: finalEspacoDet + 8, y2: alturaSecao, x: larguraDoFormulario, doc, ajusteX, ajusteY, margemEsquerda, margemTopo });
